...
class Second(http.server.BaseHTTPRequestHandler):
    ...
    def default_response(s):
        with s.server.tracer.start_active_span('default_response') as scope:
            ...
            scope.span.log_kv({'event': 'time-is-retrieved', 'second': str(now)})
            s.send_response(200)
            scope.span.log_kv({'event': 'response-status-is-sent'})
            s.send_header('Content-Type', 'text/plain')
            s.end_headers()
            scope.span.log_kv({'event': 'headers-are-sent'})
            time.sleep(0.1)
        with s.server.tracer.start_active_span('format_response') as scope:
            scope.span.log_kv({'event': 'format-response', 'second': str(now.second)})
            s.wfile.write(str.encode('%02d' % (now.second)))
...
if __name__ == '__main__':
    ...
    httpd = http.server.HTTPServer(('0.0.0.0', listen_port), Second)
    httpd.ready = True
    httpd.tracer = tracer
