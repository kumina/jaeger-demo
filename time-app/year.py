...
class Year(http.server.BaseHTTPRequestHandler):
    ...
    def default_response(s):
        ...
        with s.server.tracer.start_active_span('format_response') as scope:
            scope.span.log_kv({'event': 'response', 'year': str(now.year)})
            s.wfile.write(str.encode(str(now.year)))
...
if __name__ == '__main__':
    ...
    httpd = http.server.HTTPServer(('0.0.0.0', listen_port), Year)
    httpd.ready = True
    httpd.tracer = tracer
