...
class Month(http.server.BaseHTTPRequestHandler):
    ...
    def default_response(s):
        ...
        with s.server.tracer.start_active_span('format_response') as scope:
            scope.span.log_kv({'event': 'format-response', 'month': str(now.month)})
            s.wfile.write(str.encode(str(now.month)))
...
if __name__ == '__main__':
    ...
    httpd = http.server.HTTPServer(('0.0.0.0', listen_port), Month)
    httpd.ready = True
    httpd.tracer = tracer
