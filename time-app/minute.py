...
class Minute(http.server.BaseHTTPRequestHandler):
    ...
    def default_response(s):
        ...
        with s.server.tracer.start_active_span('format_response') as scope:
            scope.span.log_kv({'event': 'format-response', 'minute': str(now.minute)})
            s.wfile.write(str.encode('%02d' % (now.minute)))
...
if __name__ == '__main__':
    ...
    httpd = http.server.HTTPServer(('0.0.0.0', listen_port), Minute)
    httpd.ready = True
    httpd.tracer = tracer
